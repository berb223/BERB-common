...
The runner wires together the four pieces of the package:
    ...
    request → prompts → AnthropicClient → parser → verifier → result

Default behaviour validates every URL the LLM returns and drops unreachable
ones — the named "Verified" promise of the module. Pass
``validate_links=False`` to skip the network probes (faster; useful in tests
or when the consumer does its own URL filtering downstream).
"""
...
def run_verified_step(
    ...
    *,
    ...
    Returns:
        ...
        ``error_message``, ``raw_response``, and ``llm_response``.
    """
